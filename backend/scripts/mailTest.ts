...
const randomOrderId = () => `rk_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    ...
    try {
      switch (task.type) {
        ...
        case "order": {
          ...
          console.log(
            `  └─ Order Confirmation -> ${task.to} (Order #${orderId.slice(-8).toUpperCase()}, Amount ₹${(task.amount / 100).toFixed(2)})`,
          );
        ...
        case "magic-link":
          ...
          await sendPasswordlessLoginEmail(
            task.to,
            task.name,
            task.link,
            task.expiresInMinutes ?? 15,
          );
          break;

...
export function saveCustomerSession(data: { phone: string; name: string; profileToken: string; }) {
  const session = { ...data, exp: Date.now() + 7 * 24 * 60 * 60 * 1000 };
  localStorage.setItem("customer_session", JSON.stringify(session));
}

export function getCustomerSession(): { phone: string; name: string; profileToken: string; exp: number; } | null {
  try {
...
export function LoginForm({ 
  ...
  }, [checking, user, isAdmin, mfaSatisfied, navigate, emailStage, startSecondStep, redir, role, onSuccessProp]);
  ...
  const showSharedFooter = (tab === "email" && emailStage === "credentials" && !isEmailSignedIn) || (tab === "whatsapp" && waStage === "phone");
          ...
          <div className="space-y-4">
            {waStage === "phone" ? (
              ...
              <form onSubmit={waOtpForm.handleSubmit(onWaOtpSubmit)} className="space-y-4">
                ...
                <Button type="submit" variant="hero" className="w-full rounded-full" disabled={busy}>
                  {busy ? <Loader2 className="size-4 animate-spin" /> : null} Verify & log in
                </Button>
                <Button
                  type="button"
                  variant="glass"
              ...
              <form onSubmit={totpForm.handleSubmit(onTotpSubmit)} className="space-y-4">
                ...
                <div>
                  ...
                  {totpForm.formState.errors.otp && (
                    <p className="text-xs text-destructive mt-1">{totpForm.formState.errors.otp.message}</p>
                  )}
                </div>
                <Button type="submit" variant="hero" className="w-full rounded-full" disabled={busy}>
        ...
        {showSharedFooter && (
          <div className="space-y-4 pt-2">
            ...
            <p className="text-center text-sm text-muted-foreground mt-4">
              Don't have an account? <Link to="/signup" search={{ redir }} className="text-primary hover:underline font-medium">Sign up</Link>
            </p>
          </div>
        )}
